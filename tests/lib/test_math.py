...
def test_FT():
    ...
    sin = np.sin(5 * x)
    t, sin_FT = spectrumkit.lib.math.FT(x, sin)
...
def test_iFT():
    ...
    sin = np.sin(5 * x)
    t, sin_FT = spectrumkit.lib.math.FT(x, sin)
    sin_new = spectrumkit.lib.math.iFT(t, sin_FT, indvar=False)
...
def test_iFT_unequal_spacing():
    ...
    with pytest.raises(ValueError, match=match):
        spectrumkit.lib.math.iFT(t, sin)
